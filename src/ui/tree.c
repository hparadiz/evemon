                  ...
                  ui_ctx_t           *ctx)
{
    ...
    for (size_t j = 0; j < new_count; j++) {
        ...
        while (sp > 0) {
            ...
            /* Expand the parent the moment its first child arrives,
             * unless the user has manually collapsed it. */
            if (parent_iter && ctx &&
                gtk_tree_model_iter_n_children(
                    GTK_TREE_MODEL(store), parent_iter) == 1 &&
                !node_pid_is_collapsed(&ctx->collapsed_pids, e->ppid)) {
